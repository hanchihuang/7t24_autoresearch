...
This repository is a working export of a GRPO-on-GSM8K autoresearch setup.

It contains two layers:

- The upstream `codex-autoresearch` skill/runtime as the unattended experimentation engine.
- A concrete GRPO/GSM8K snapshot under [`grpo_gsm8k_snapshot/`](grpo_gsm8k_snapshot/) with the watchdog, launch/state files, main training script, and the latest retained progress logs.
...
- Current runtime status: `needs_human`
- Current next-step recommendation: `reward-side / slice-aware shaping`

### Included snapshot files

- [`grpo_gsm8k_snapshot/llama3_1_(8b)_grpo.py`](grpo_gsm8k_snapshot/llama3_1_(8b)_grpo.py)
- [`grpo_gsm8k_snapshot/grpo_autoresearch_watchdog.py`](grpo_gsm8k_snapshot/grpo_autoresearch_watchdog.py)
- [`grpo_gsm8k_snapshot/autoresearch-launch.json`](grpo_gsm8k_snapshot/autoresearch-launch.json)
- [`grpo_gsm8k_snapshot/autoresearch-state.json`](grpo_gsm8k_snapshot/autoresearch-state.json)
- [`grpo_gsm8k_snapshot/autoresearch-runtime.json`](grpo_gsm8k_snapshot/autoresearch-runtime.json)
...
- Local retained best continuation line: `0.48 (96/200)`
- Earlier confirm200 verifier-aware kept line: `0.475 (95/200)`
- The latest failed scouts after the `0.48` keep were:
  - `iter85 percentage_focus -> 0.20`
  - `iter87 difficulty_prompt_replay -> 0.20`

The main reason the loop is paused is not a crash. It is a strategic soft block: the current family of narrower synthetic-template sweeps and lightweight replay approximations stopped producing signal.
